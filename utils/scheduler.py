...
class Scheduler:
    ...
    def __init__(
        ...
        scaling_factor = np.array(
            [
                1
                - scaling_derivative_fn(time_steps[i]) / scaling_fn(time_steps[i]) * (time_steps[i] - time_steps[i + 1])
        ...
        self.sigma_steps, self.time_steps, self.factor_steps, self.scaling_factor, self.scaling_steps = (
            sigma_steps,
            time_steps,
            factor_steps,
            scaling_factor,
    ...
    def get_sigma_fn(self, schedule):
        ...
        if schedule == "sqrt":
            ...
            sigma_derivative_fn = (
                lambda t: (beta_d * t + beta_min) * np.exp(beta_d * t**2 / 2 + beta_min * t) / 2 / sigma_fn(t)
            )
            sigma_inv_fn = (
